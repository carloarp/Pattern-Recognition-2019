...
def main():	
	...
	for M1 in range(15,415,15):
		...
		for ntest in range(0,test_size):
			...
			for t in range(0,T):
				...
				print("M1 =",M1,", Test data =",ntest+1,"/",test_size,", Classifier =",t+1,",Prediction =",label,end="\r")			
	...
	ensemble_df = pd.DataFrame(columns=['M1','Majority Success Rate','Average Success Rate','Sum Success Rate'])
	ensemble_df['M1'] = M1_list
	...
	print(ensemble_df)
	plot_3df(ensemble_df,plot_title='M1 against different Success Rates while keeping M0=100',x_axis='M0',y_axis='Success Rate (%)',plot='yes',save='no')
