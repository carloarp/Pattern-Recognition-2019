...
def PCA_LDA_Sum_classifier(X_train,X_test,Y_train,Y_test,Wopt,M0,M1,Mlda,T):
	...
	title = 'PCA-LDA Sum Confusion Matrix with T='+str(T)+' ,M0='+str(M0)+' ,M1='+str(M1)+' [Success Rate='+str(success_rate)+'%]'
	plot_confusion_matrix(Y_test, y_predicted,M0,M1,Mlda,success_rate,main_title=title,show='no')

	return success_rate

def plot_confusion_matrix(y_true, y_predicted, M0, M1, Mlda, success_rate, main_title, show):
    ...
    if (show == 'yes'):
        ...
        print('\n')
        print('Mlda =',Mlda,'M0 =',M0,' , M1 =',M1,'Number of true positives = ', true_positives)
        plt.close()	
...
def main():	
	...
	for Mlda in range(40,52,1):	
		...
		for ntest in range(0,test_size):
			result_list = []
			individual_model_accuracy_list = []
		...
		success_rate = success/test_size*100
		success_rate = np.round(success_rate,3)
		title = 'PCA-LDA Majority Vote Confusion Matrix with T='+str(T)+' ,Mlda='+str(Mlda)+' ,M1='+str(M1)+' [Success Rate='+str(success_rate)+'%]'
		maybe = 'no'

		plot_confusion_matrix(Y_test, y_predicted,M0,M1,Mlda,success_rate,main_title=title,show=maybe)
		...
		sum_success_rate = PCA_LDA_Sum_classifier(X_train,X_test,Y_train,Y_test,Wpcalda_sum,M0,M1,Mlda,T)
		print("Sum Success Rate for T =",T," is ",sum_success_rate,"%")
		print('------------------------------------------------------')
		
		vote_success_list.append(success_rate)
		sum_success_list.append(individual_success_rate)
		average_success_list.append(sum_success_rate)
		Mlda_list.append(Mlda)
	
	df = pd.DataFrame(columns=[])
	df['Mlda'] = Mlda_list
	df['Majority Success Rate'] = vote_success_list
	df['Average Success Rate'] = sum_success_list
	df['Sum Success Rate'] = average_success_list
	...
	plot_3df(df,plot_title='none',x_axis='Mlda',y_axis='Success Rate(%)',plot='yes',save='no')
	#sys.exit()
