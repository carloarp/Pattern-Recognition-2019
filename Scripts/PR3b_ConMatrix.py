...
def PCA_LDA_Sum_classifier(X_train,X_test,Y_train,Y_test,Wopt,M0,M1,T):
	...
	plot_confusion_matrix(Y_test, y_predicted,M0,M1,success_rate,main_title=title,show='yes')
...
def main():	
	...
	success_rate = success/test_size*100
	success_rate = np.round(success_rate,3)
	title = 'PCA-LDA Majority Vote Confusion Matrix with T='+str(T)+' ,M0='+str(M0)+' ,M1='+str(M1)+' [Success Rate='+str(success_rate)+'%]'
	plot_confusion_matrix(Y_test, y_predicted,M0,M1,success_rate,main_title=title,show='yes')
	...
	sum_success_rate = PCA_LDA_Sum_classifier(X_train,X_test,Y_train,Y_test,Wpcalda_sum,M0,M1,T)
