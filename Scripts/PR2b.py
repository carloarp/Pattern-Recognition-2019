...
def main():	
	...
	merged_x, batch_mean = calculate_equivalent_batch_mean(subset_list,mode='none')
	A = np.subtract(merged_x,combined_subset_mean).squeeze()
	A = A.T
	#print(A)
	#print(A.shape)
	
	#### CALCULATE COMBINED COVARIANCE MATRIX: SUBSET_1 + SUBSET_2
	S_combined = calculate_combined_covariance(x_subset1,x_subset2,subset1_mean_face,subset2_mean_face,mode='print')
