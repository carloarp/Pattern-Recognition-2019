...
def main():	
	...
	for M0 in range(50,401,50):
		...
		for ntest in range(0,test_size):
			...
			for t in range(0,T):
				...
				print("M0 =",M0,", Test data =",ntest+1,"/",test_size,", Classifier =",t+1,",Prediction =",label,end="\r")			
	...
	ensemble_df = pd.DataFrame(columns=['M0','Majority Success Rate','Average Success Rate','Sum Success Rate'])
	ensemble_df['M0'] = M0_list
	...
	print(ensemble_df)
	plot_3df(ensemble_df,plot_title='M0 against different Success Rates while keeping M1=15',x_axis='M0',y_axis='Success Rate (%)',plot='yes',save='no')
