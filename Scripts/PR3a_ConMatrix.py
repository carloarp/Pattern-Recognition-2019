#### Plots the Confusion Matrix for PCA-LDA best recognition rate
...
def PCA_LDA_classifier(X_train,X_test,Y_train,Y_test,Wopt,Mpca,Mlda):
		...
		plot_confusion_matrix(Y_test, y_predicted,Mpca,Mlda,success_rate,show='yes')
...
def main():	
	...
	list3 = list(range(100, matrix_rank(Sw), 20))
	list3.append(matrix_rank(Sw))
	Mpca_list = list1+list2+list3
	success_rate_list = []
	
	df = pd.DataFrame(columns=['Mpca','Success Rate'])
	W_pca = Wpca
	Mpca_list = [364]
	for Mpca in Mpca_list:
		...
		success_rate = PCA_LDA_classifier(X_train,X_test,Y_train,Y_test,Wopt,Mpca,Mlda)
		success_rate_list.append(success_rate)
		print("For Mpca =",Mpca,"and Mlda =",Mlda,", success rate is",success_rate,"%")	
		
	#df['Success Rate'] = success_rate_list
	#df['Mpca'] = Mpca_list
	#plot_df(df,plot_title='Recognition Rate while varying Mpca',x_axis='Mpca',y_axis='Recognition Rate%',plot='yes',save='yes')
