...
def calculate_Sw(x_class,x_class_mean,show='yes'):
	Sw = [0]
	for i in range(0,52):
		x_mi = np.subtract(x_class[0],x_class_mean[0]).T
		Si = np.dot(x_mi,x_mi.T)
		Sw = Sw + Si
		
	if show == 'yes':
		print("Sw has shape ",Sw.shape,':\n',Sw,'\n')
...
def calculate_Wpca(x_train,global_mean,show):
	...
	S_pca = calculate_batch_covariance_matrix(x_train,global_mean)
	Wpca = get_sorted_eigenvectors(S_pca)
...
def main():	
	...
	Sw = calculate_Sw(x_class,x_class_mean,show='no')
	
	#### MULTIPLY Sw_inverse and Sb
	Sw_inv = inv(Sw)
	SwSb = np.dot(Sw_inv,Sw)
	#print("Sw_inv.Sb has shape",SwSb.shape,':\n',SwSb,'\n')

	#### COMPUTE Wlda
	Wlda = calculate_Wlda(SwSb,show='no')
	
	#### COMPUTE Wpca
	Wpca = calculate_Wpca(x_train,global_mean,show='no')
	
	#### COMPUTE Wopt
	Wopt = calculate_Wopt(Wlda,Wpca,show='no')
	
	#### PROJECT TRAINING DATA ON 3D EIGENSPACE
	Wopt = Wopt[:,0:3]
	#print(Wopt)
	#print(x_class[0][0])
	#print(x_class[0][0].shape)
	
	A_train = np.subtract(x_train,global_mean).squeeze()
	A_train = A_train.T
	
	A = []
	W_train = []
	for i in range(0,52):
		Ai = np.subtract(x_class[i],x_class_mean[i]).T
		A.append(Ai)
		Wi = np.dot(x_class[i],Wopt)
		#Wi = np.dot(Ai.T,Wopt) 
		W_train.append(Wi)
	
	#print(W_train[0])
	#print(W_train[0][0][0])		### [class=0,1,2,...,52][index=0,1,2...,8][axis=0,1,2]
	
	from matplotlib.pyplot import cm
	color=iter(cm.rainbow(np.linspace(0,4,53)))
	...
	for Class in range(0,10):
		...
		for index in range(0,8):
			x = W_train[Class][index][0]
			y = W_train[Class][index][1]
			z = W_train[Class][index][2]
